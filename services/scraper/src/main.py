...
def parse_feed_and_save(db, redis_conn, portal):
    ...
    try:
        feed = feedparser.parse(portal.rss_feed_url, request_headers={'User-Agent': '2026-Scraper-Bot/1.0'})
    except Exception as e:
        log.error(f"Feed letöltési hiba ({portal.name}): {e}")
        return 0

    if feed.bozo:
        log.warning(f"Hibás feed formátum: {portal.name} (Hiba: {feed.bozo_exception})")
        return 0
    ...
    for entry in feed.entries:
        ...
        try:
            ...
            log.info(f"ÚJ CIKK: {portal.name} - {entry.title}")
            new_count += 1
